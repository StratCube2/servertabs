...
public class TabConfig {
    ...
    public enum TransitionSpeed {
        SLOW(0.05f, "Slow"), MEDIUM(0.10f, "Medium"), FAST(0.20f, "Fast");
    ...
    public TabEntry getDefaultTab() {
        return tabs.stream().filter(t -> t.getId().equals(defaultTabId)).findFirst().orElse(tabs.get(0));
    }
    ...
    // Mutations (Manual save required by caller for batching)
    ...
    public void renameTab(TabEntry tab, String newName) {
        if (!tab.isLocked()) tab.setName(newName);
    }
    ...
    public void moveTabUp(int index) {
        if (index > 0 && index < tabs.size() && !tabs.get(index - 1).isLocked()) Collections.swap(tabs, index, index - 1);
    }

    public void moveTabDown(int index) {
        if (index >= 0 && index < tabs.size() - 1 && !tabs.get(index).isLocked()) Collections.swap(tabs, index, index + 1);
    }
    ...
    public void assignServer(String serverIp, String tabId) {
        ...
        if (!key.isEmpty()) serverTabAssignments.computeIfAbsent(key, k -> new HashSet<>()).add(tabId);
    }
    ...
    public boolean serverInTab(String serverIp, String tabId) {
        ...
        Set<String> ids = serverTabAssignments.get(normaliseIp(serverIp));
